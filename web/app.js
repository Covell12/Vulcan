...
// describeFetchError() and errorText() now live in api.js (shared by both flows).
...
async function loadTemplates() {
  ...
  try {
    const response = await VulcanAPI.getTemplates();
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await VulcanAPI.createDesign({
    ...
    previewImg.src = `${VulcanAPI.asset(design.files.preview_png)}?t=${Date.now()}`;
